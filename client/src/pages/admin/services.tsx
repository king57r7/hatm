...
import {
  ...
  RefreshCw, Search, SlidersHorizontal, Star, Tags, Trash2, X, Zap,
} from 'lucide-react';
import { useLang } from '../../lib/context';

type BulkAction = 'activate' | 'deactivate' | 'hide' | 'show' | 'activate_featured' | 'delete' | 'markup';
...
export default function AdminServicesPage() {
  ...
  const [editVals, setEditVals] = useState({ name: '', nameAr: '', category: '', catAr: '', description: '', descriptionAr: '', imageUrl: '', type: '', min: '', max: '', markup: '', refill: false, cancel: false });
  ...
  const openEditor = (service: any) => {
    setEditing(service);
    setEditVals({ name: service.name || '', nameAr: service.nameAr || '', category: service.category || '', catAr: service.categoryAr || '', description: service.description || '', descriptionAr: service.descriptionAr || '', imageUrl: service.imageUrl || '', type: service.type || '', min: String(service.min ?? ''), max: String(service.max ?? ''), markup: String(service.markupPercent ?? ''), refill: Boolean(service.refill), cancel: Boolean(service.cancel) });
  };

  useEffect(() => {
    const serviceId = new URLSearchParams(window.location.search).get('edit');
    if (!serviceId || editing) return;
    fetch(`/api/services/${serviceId}`, { credentials: 'include' }).then(response => response.ok ? response.json() : null).then(data => {
      if (data?.service) openEditor(data.service);
    });
  }, [services.length]);
  ...
  const handleImage = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => setEditVals(current => ({ ...current, imageUrl: String(reader.result || '') }));
  ...
  return <div className="page-shell">
    ...
    {editing && <div className="fixed inset-0 z-[60] flex items-end bg-[#03040b]/78 p-0 backdrop-blur-sm sm:items-center sm:justify-center sm:p-5"><section className="w-full rounded-t-[1.7rem] border border-white/[.1] bg-[#12182a] shadow-2xl sm:max-w-2xl sm:rounded-[1.5rem]" role="dialog" aria-modal="true"><div className="flex items-start justify-between border-b border-white/[.07] p-5"><div><div className="eyebrow"><span className="eyebrow-dot" />{locale === 'ar' ? 'تحرير كامل للخدمة' : 'Full service editor'}</div><h3 className="mt-2 max-w-[22rem] truncate text-lg font-extrabold text-white">{editing.name}</h3></div><button onClick={() => setEditing(null)} className="icon-button"><X size={18} /></button></div><div className="max-h-[68vh] space-y-4 overflow-y-auto p-5"><div className="grid gap-4 sm:grid-cols-2"><div><label className="input-label">{locale === 'ar' ? 'الاسم بالإنجليزية' : 'English name'}</label><input value={editVals.name} onChange={event => setEditVals({ ...editVals, name: event.target.value })} className="input-field" /></div><div><label className="input-label">{locale === 'ar' ? 'الاسم بالعربية' : 'Arabic name'}</label><input value={editVals.nameAr} onChange={event => setEditVals({ ...editVals, nameAr: event.target.value })} className="input-field" dir="rtl" /></div><div><label className="input-label">{locale === 'ar' ? 'التصنيف بالإنجليزية' : 'English category'}</label><input value={editVals.category} onChange={event => setEditVals({ ...editVals, category: event.target.value })} className="input-field" /></div><div><label className="input-label">{locale === 'ar' ? 'التصنيف بالعربية' : 'Arabic category'}</label><input value={editVals.catAr} onChange={event => setEditVals({ ...editVals, catAr: event.target.value })} className="input-field" dir="rtl" /></div><div className="sm:col-span-2"><label className="input-label">{locale === 'ar' ? 'الوصف بالإنجليزية' : 'English description'}</label><textarea value={editVals.description} onChange={event => setEditVals({ ...editVals, description: event.target.value })} className="input-field min-h-20" /></div><div className="sm:col-span-2"><label className="input-label">{locale === 'ar' ? 'الوصف بالعربية' : 'Arabic description'}</label><textarea value={editVals.descriptionAr} onChange={event => setEditVals({ ...editVals, descriptionAr: event.target.value })} className="input-field min-h-20" dir="rtl" /></div><div><label className="input-label">{locale === 'ar' ? 'نوع الطلب' : 'Order type'}</label><input value={editVals.type} onChange={event => setEditVals({ ...editVals, type: event.target.value })} className="input-field" placeholder="Default / Package / Link" /></div><div><label className="input-label">{t.markupPercent}</label><input type="number" value={editVals.markup} onChange={event => setEditVals({ ...editVals, markup: event.target.value })} className="input-field mono" /></div><div><label className="input-label">{locale === 'ar' ? 'الحد الأدنى' : 'Minimum'}</label><input type="number" value={editVals.min} onChange={event => setEditVals({ ...editVals, min: event.target.value })} className="input-field mono" /></div><div><label className="input-label">{locale === 'ar' ? 'الحد الأقصى' : 'Maximum'}</label><input type="number" value={editVals.max} onChange={event => setEditVals({ ...editVals, max: event.target.value })} className="input-field mono" /></div></div><div className="flex flex-wrap gap-4 text-sm text-white"><label className="flex items-center gap-2"><input type="checkbox" checked={editVals.refill} onChange={event => setEditVals({ ...editVals, refill: event.target.checked })} />{locale === 'ar' ? 'إعادة تعبئة' : 'Refill'}</label><label className="flex items-center gap-2"><input type="checkbox" checked={editVals.cancel} onChange={event => setEditVals({ ...editVals, cancel: event.target.checked })} />{locale === 'ar' ? 'قابل للإلغاء' : 'Cancelable'}</label></div><div className="rounded-xl border border-white/[.08] p-3"><label className="input-label">{locale === 'ar' ? 'صورة الخدمة' : 'Service image'}</label><input ref={imageInputRef} type="file" accept="image/*" onChange={handleImage} className="mt-2 block w-full text-xs text-[#a9b5d4]" />{editVals.imageUrl && <img src={editVals.imageUrl} alt="" className="mt-3 h-24 w-32 rounded-xl object-cover" />}</div></div><div className="flex gap-3 border-t border-white/[.07] p-5"><button onClick={() => setEditing(null)} className="btn-secondary flex-1">{t.cancel}</button><button onClick={() => update(editing.id, { name: editVals.name, nameAr: editVals.nameAr, category: editVals.category, categoryAr: editVals.catAr, description: editVals.description, descriptionAr: editVals.descriptionAr, imageUrl: editVals.imageUrl, type: editVals.type, min: Number(editVals.min), max: Number(editVals.max), refill: editVals.refill, cancel: editVals.cancel, markupPercent: Number(editVals.markup) })} className="btn-primary flex-1">{t.save}</button></div></section></div>}
