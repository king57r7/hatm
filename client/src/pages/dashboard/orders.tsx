...
export default function OrdersPage() {
  ...
  useEffect(() => {
    fetch('/api/orders', { credentials: 'include' }).then(r => r.json()).then(d => setOrders(d.orders || [])).finally(() => setLoading(false));
  }, []);
        ...
        <div className="card overflow-hidden p-0">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-[#0d0d0d]"><tr>{[t.services, t.link, t.quantity, t.price, t.status, t.date].map(h => <th key={h} className="table-header">{h}</th>)}</tr></thead>
              <tbody>
                {orders.map(o => (
                  <tr key={o.id} className="table-row">
                    <td className="table-cell text-white font-medium max-w-[200px] truncate">{o.service?.name}</td>
                    <td className="table-cell max-w-[150px] truncate"><a href={o.link} target="_blank" rel="noreferrer" className="text-amber-500 hover:underline">{o.link}</a></td>
