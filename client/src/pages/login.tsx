        ...
        <div className="card">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="input-field" placeholder="example@email.com" required />
            </div>
            <div>
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full py-3 text-base">{loading ? t.loading : t.login}</button>
          </form>
          <p className="text-center text-gray-500 mt-6 text-sm">
            {t.dontHaveAccount}{' '}
            <a href="/register" onClick={e => { e.preventDefault(); navigate('/register'); }} className="text-amber-500 hover:text-amber-400 font-semibold">{t.register}</a>
          </p>
        </div>
      </div>
    </div>
